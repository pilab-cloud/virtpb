...
type Task struct {
	...
	// vm_id is the cluster uid of the target VM once one has been assigned
	// (PIVIRT-84, Phase 3) -- see control-plane-architecture.md §2.5 / D4's
	// control loop, "NodeDirective{Task, uid, generation, spec}". Empty for
	// the initial VM_CREATE task, whose uid the director mints only after
	...
	// generation is the spec generation this task is reconciling the VM
	// toward (PIVIRT-84, Phase 3). The agent persists it alongside vm_id on
	// successful completion, so observed_generation can be reported back on
	// the next resync/status event.
	Generation    uint64 `protobuf:"varint,19,opt,name=generation,proto3" json:"generation,omitempty"`
...
const file_pilab_director_v2_task_service_proto_rawDesc = "" +
	"\n" +
	"$pilab/director/v2/task_service.proto\x12\x11pilab.director.v2\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x1cgoogle/protobuf/struct.proto\"\xea\x05\n" +
	"\x04Task\x12\x0e\n" +
	...
	"\x06reason\x18\x12 \x01(\tR\x06reason\x12\x1e\n" +
	"\n" +
	"generation\x18\x13 \x01(\x04R\n" +
	"generation\"g\n" +
